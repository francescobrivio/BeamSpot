...
void dxy_fitter ()
{
	...
	TFile* outfile = TFile::Open("graphs3.root","RECREATE");
	...
	//for (int i = 0; i < 10000; ++i) 
	{
		...
		std::cout<<"Accetted track from vertex: "<<iVtxID<<std::endl;
		...
		for (int j = i+1; j< nentries; ++j)
		{
			...
			if (iVtxID == tmp_vtx+1) break;

			// Track + vertex selection
			if ( (iVtxID != tmp_vtx) ) continue;//|| (ipt < pt_cut) || (std::abs(ieta)> eta_cut) || (iTrack_HITs < trk_hits_cut) || (iPix_HITs < pix_hits_cut) ) continue; 
				//|| (iQuality != 1) || (ichi2 > 10.) || (ichi2 < 0.) ) continue;
	...
	// Fit the TProfiles
	std::cout<<"Fitting...\n";
   	prof_cos_minus->Fit("pol1");
   	prof_cos_plus ->Fit("pol1");
	...
	TF1 *fit_minus = prof_cos_minus->GetFunction("pol1");
	double q_minus = fit_minus->GetParameter(0);
	double m_minus = fit_minus->GetParameter(1);

	TF1 *fit_plus = prof_cos_plus->GetFunction("pol1");
